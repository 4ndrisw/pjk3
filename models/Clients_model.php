...
class Clients_model extends App_Model
{
     ...
     * @return array
     */
    public function get_user_ispjk3ornot ($customer_id)
    {
        $this->db->select('userid');
        $this->db->where('id',$customer_id);
        $contactuid= $this->db->get(db_prefix() . 'contacts')->result_array();


        $this->db->select('is_pjk3');
        $this->db->where('userid',$contactuid[0]['userid']);
        $contactispjk3 = $this->db->get(db_prefix() . 'clients')->result_array();



        // print_r( $contactispjk3[0]['is_pjk3']); exit();

        return $contactispjk3[0]['is_pjk3'];
    }
     ...
     * @param  integer Status ID
     * @return boolean
     * Update client status prefferd/not prefferd
     */
    public function change_pjk3_preference($id, $status)
    {
        ...
        if ($this->db->affected_rows() > 0) {
            log_activity('Change pjk3 preference Status Changed [ID: ' . $id . ' Status(Active/Inactive): ' . $status . ']');
    ...
    */
    ...
    public function send_notification_customer_profile_file_uploaded_to_responsible_staff($contact_id, $customer_id)
    {
        ...
        foreach ($staff as $member) {
            ...
            if (add_notification([
                    ...
                    'link' => 'pjk3/client/' . $customer_id . '?group=attachments',
                ])) {
    ...
    private function check_zero_columns($data)
    {
        ...
        if (isset($data['is_pjk3']) && $data['is_pjk3'] != '') {
            $data['is_pjk3'] = 1;
        }
        else $data['is_pjk3'] = 0;
