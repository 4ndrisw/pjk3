            ...
            <ul class="nav nav-tabs profile-tabs row customer-profile-tabs nav-tabs-horizontal" role="tablist">
               <li role="presentation" class="<?php if (!$this->input->get('tab')) {
                  ...
                  <a href="#contact_info" aria-controls="contact_info" role="tab" data-toggle="tab">
                     <?php echo _l('pjk3_profile_details'); ?>
                  </a>
               </li>
               <?php
      ...
      <div class="tab-content">
         ...
         <div role="tabpanel" class="tab-pane<?php if (!$this->input->get('tab')) {
            ...
            <div class="row">
               <div class="col-md-12<?php if (isset($client) && (!is_empty_customer_company($client->userid) && total_rows(db_prefix() . 'contacts', array('userid' => $client->userid, 'is_primary' => 1)) > 0)) {
                  ...
                  <div class="checkbox checkbox-info mbot20 no-mtop fgfhfgdfg">
                  ...
                  <div class="checkbox checkbox-primary no-mtop checkbox-inline task-add-edit-public" style=" display:none;">
                     <input type="checkbox" id="is_pjk3" name="is_pjk3" checked>
                     <label for="is_pjk3"><?= _l('is_pjk3') ?></label>
                  </div>
               </div>
               <div class="col-md-6">
                  ...
                  <?php if (get_option('pjk3_use_bpjs_kesehatan_field') == 1) {
                     $value = (isset($client) ? $client->bpjs_kesehatan : '');
                     echo render_input('bpjs_kesehatan', 'pjk3_bpjs_kesehatan', $value);
                  } ?>
                  <?php if (get_option('pjk3_use_bpjs_ketenagakerjaan_field') == 1) {
                     $value = (isset($client) ? $client->bpjs_ketenagakerjaan : '');
                     echo render_input('bpjs_ketenagakerjaan', 'pjk3_bpjs_ketenagakerjaan', $value);
                  } ?>

                  <?php
                  //if (get_option('pjk3_use_siup_resiko_field') == 1) {
                     $value = (isset($client) ? $client->siup_resiko : '');
                     echo render_input('siup_resiko', 'pjk3_siup_resiko', $value);
                  //} 
                  ?>
                  ...
                  ?>
                  <?php if (!isset($client)) { ?>
                     <i class="fa fa-question-circle pull-left" data-toggle="tooltip" data-title="<?php echo _l('pjk3_currency_change_notice'); ?>"></i>
                  <?php }
                  ...
                  <tbody>
                     <?php foreach ($customer_admins as $c_admin) { ?>
                        <tr>
                           ...
                           <?php if (has_permission('customers', '', 'create') || has_permission('customers', '', 'edit')) { ?>
                              <td>
                                 <a href="<?php echo admin_url('pjk3/delete_customer_admin/' . $client->userid . '/' . $c_admin['staff_id']); ?>" class="btn btn-danger _delete btn-icon"><i class="fa fa-remove"></i></a>
                              </td>
                           <?php } ?>
                        </tr>
                     <?php } ?>
         ...
         <div role="tabpanel" class="tab-pane" id="billing_and_shipping">
            <div class="row">
               <div class="col-md-12">
                  <div class="row">
                     <div class="col-md-6 dsdas">
                        <h4 class="no-mtop"><?php echo _l('billing_address'); ?> <a href="#" class="pull-right billing-same-as-customer"><small class="font-medium-xs"><?php echo _l('pjk3_billing_same_as_profile'); ?></small></a></h4>
                        <hr />
                     ...
                     <div class="col-md-6">
                        <h4 class="no-mtop">
                           <i class="fa fa-question-circle" data-toggle="tooltip" data-title="<?php echo _l('pjk3_shipping_address_notice'); ?>"></i>
                     ...
                     <?php if (
                        isset($client) && (total_rows(db_prefix() . 'invoices', array('clientid' => $client->userid)) > 0 || total_rows(db_prefix() . 'estimates', array('clientid' => $client->userid)) > 0 || total_rows(db_prefix() . 'creditnotes', array('clientid' => $client->userid)) > 0)
                     ) { ?>
                        <div class="col-md-12">
...
<?php if (isset($client)) { ?>
   <?php if (has_permission('customers', '', 'create') || has_permission('customers', '', 'edit')) { ?>
      <div class="modal fade" id="customer_admins_assign" tabindex="-1" role="dialog">
         <div class="modal-dialog">
            <?php echo form_open(admin_url('pjk3/assign_admins/' . $client->userid)); ?>
   ...
   <?php } ?>
<?php } ?>
<?php $this->load->view('admin/pjk3/client_group'); ?>