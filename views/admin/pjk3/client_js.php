...
$(function() {
    ...
    <?php } ?>
    _table_api = initDataTable('.table-contacts', admin_url + 'pjk3/contacts/' + customer_id, contactsNotSortable, contactsNotSortable);
    if(_table_api) {
          <?php if(is_gdpr() && get_option('gdpr_enable_consent_for_contacts') == '1'){ ?>
   ...
   initDataTable('.table-credit-notes', admin_url+'credit_notes/table/'+customer_id, ['undefined'], ['undefined'], undefined, [0, 'desc']);
    ...
    if (app.options.pjk3_bpjs_kesehatan_is_required == 1) {
        vRules = {
            bpjs_kesehatan: 'required',
        }
    }
    if (app.options.pjk3_bpjs_ketenagakerjaan_is_required == 1) {
        vRules = {
    ...
    if(typeof(customer_id) == 'undefined'){
        $('#company').on('blur', function() {
            ...
            $.post(admin_url+'pjk3/check_duplicate_customer_name', {company:company})
...
function delete_contact_profile_image(contact_id) {
    requestGet('pjk3/delete_contact_profile_image/'+contact_id).done(function(){
...
function saveCustomerProfileExternalFile(files, externalType) {
    $.post(admin_url + 'pjk3/add_external_attachment', {
        files: files,
...
function contact(client_id, contact_id) {
    ...
    requestGet('pjk3/form_contact/' + client_id + '/' + contact_id).done(function(response) {
...
function update_all_proposal_emails_linked_to_contact(contact_id) {
    ...
    $.post(admin_url + 'pjk3/update_all_proposal_emails_linked_to_customer/' + contact_id, data).done(function(response) {
        response = JSON.parse(response);
        if (response.success) {
            alert_float('success', response.message);
        }
...
function do_share_file_contacts(edit_contacts, file_id) {
    ...
    $.post(admin_url + 'pjk3/update_file_share_visibility', {
        file_id: file_id,
...
function save_longitude_and_latitude(clientid) {
    ...
    $.post(admin_url + 'pjk3/save_longitude_and_latitude/'+clientid, data).done(function(response) {
       if(response == 'success') {
            alert_float('success', "<?php echo _l('updated_successfully', _l('pjk3')); ?>");
       }
        setTimeout(function(){
            window.location.reload();
        },1200);
