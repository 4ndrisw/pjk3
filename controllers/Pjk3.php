...
class Pjk3 extends AdminController
{
    ...
    public function index()
    {
        if (!has_permission('pjk3', '', 'view')) {
            if (!have_assigned_customers() && !has_permission('pjk3', '', 'create')) {
                access_denied('pjk3');
            }
        }
        ...
        $this->load->view('admin/pjk3/manage', $data);
    }
    ...
    public function table()
    {
        ...
        $this->app->get_table_data(module_views_path(MODULE_PJK3, 'tables/pjk3'));
    ...
    public function all_contacts()
    {
        if ($this->input->is_ajax_request()) {
            // $this->app->get_table_data('all_contacts');
             $this->app->get_table_data(module_views_path(MODULE_PJK3, 'tables/all_contacts'));
        }
        ...
        $this->load->view('admin/pjk3/all_contacts', $data);
    }
    ...
    public function client($id = '')
    {
            ...
            if ($id == '') {
                ...
                if ($id) {
                    ...
                    if ($save_and_add_contact == false) {
                        redirect(admin_url('pjk3/client/' . $id));
                    } else {
                        redirect(admin_url('pjk3/client/' . $id . '?group=contacts&new_contact=true'));
                    }
                }
            } else {
                if (!has_permission('pjk3', '', 'edit')) {
                ...
                if ($success == true) {
                    set_alert('success', _l('updated_successfully', _l('pjk3')));
                }
                redirect(admin_url('pjk3/client/' . $id));
            }
        }
        ...
        if ($group != 'contacts' && $contact_id = $this->input->get('contactid')) {
            redirect(admin_url('pjk3/client/' . $id . '?group=contacts&contactid=' . $contact_id));
        }
        ...
        if ($id == '') {
            ...
            //     $data['products'] = $this->products_model->get_project_statuses();
            // }
        ...
        $this->load->view('admin/pjk3/client', $data);
    }
    ...
    public function form_contact($customer_id, $contact_id = '')
    {
        ...
        $data['title']                = $title;
        $this->load->view('admin/pjk3/modals/contact', $data);
    }
    ...
    {
        if (!has_permission('pjk3', '', 'create') && !has_permission('pjk3', '', 'edit')) {
            access_denied('customers');
        }
        ...
        redirect(admin_url('pjk3/client/' . $id . '?tab=customer_admins'));
    }
    ...
    public function delete_customer_admin($customer_id, $staff_id)
    {
        ...
        redirect(admin_url('pjk3/client/' . $customer_id) . '?tab=customer_admins');
    }

    public function delete_contact($customer_id, $id)
    {
        if (!has_permission('pjk3', '', 'delete')) {
            if (!is_customer_admin($customer_id)) {
                access_denied('customers');
            }
        }
        ...
        redirect(admin_url('pjk3/client/' . $customer_id . '?group=contacts'));
    }
    ...
    {
        if (has_permission('pjk3', '', 'delete') || is_customer_admin($customer_id)) {
    ...
    public function delete($id)
    {
        ...
        if (is_array($response) && isset($response['referenced'])) {
            set_alert('warning', _l('pjk3_delete_transactions_warning', _l('invoices') . ', ' . _l('estimates') . ', ' . _l('credit_notes')));
        } elseif ($response == true) {
            set_alert('success', _l('deleted', _l('pjk3')));
        } else {
            set_alert('warning', _l('problem_deleting', _l('pjk3_lowercase')));
        }
        redirect(admin_url('pjk3'));
    }
    ...
    {
        if (has_permission('pjk3', '', 'edit') || is_customer_admin(get_user_id_by_contact_id($id))) {
            if ($this->input->is_ajax_request()) {
    ...
    /* change pjk3 preference */
    public function change_pjk3_preference($id, $status){
        if ($this->input->is_ajax_request()) {
            $this->clients_model->change_pjk3_preference($id, $status);
        }
    }
    ...
    public function zip_credit_notes($id)
    {
        ...
        if ($this->input->post()) {
            $this->load->library('app_bulk_pdf_export', [
                ...
                'redirect_on_error' => admin_url('pjk3/client/' . $id . '?group=credit_notes'),
            ]);
    ...
    public function zip_invoices($id)
    {
        ...
        if ($this->input->post()) {
            $this->load->library('app_bulk_pdf_export', [
                ...
                'date_from'         => $this->input->post('zip-from'),
                'date_to'           => $this->input->post('zip-to'),
                'redirect_on_error' => admin_url('pjk3/client/' . $id . '?group=invoices'),
            ]);
    ...
    public function zip_estimates($id)
    {
        ...
        if ($this->input->post()) {
            $this->load->library('app_bulk_pdf_export', [
                ...
                'date_from'         => $this->input->post('zip-from'),
                'date_to'           => $this->input->post('zip-to'),
                'redirect_on_error' => admin_url('pjk3/client/' . $id . '?group=estimates'),
            ]);
    ...
    public function zip_payments($id)
    {
        ...
        $this->load->library('app_bulk_pdf_export', [
                ...
                'date_from'         => $this->input->post('zip-from'),
                'date_to'           => $this->input->post('zip-to'),
                'redirect_on_error' => admin_url('pjk3/client/' . $id . '?group=payments'),
            ]);
    ...
    public function import()
    {
        ...
        $this->load->view('admin/pjk3/import', $data);
    }
    ...
    public function groups()
    {
        ...
        $this->load->view('admin/pjk3/groups_manage', $data);
    }
    ...
    public function group()
    {
        ...
        if ($this->input->is_ajax_request()) {
            ...
            if ($data['id'] == '') {
                ...
                $message = $id ? _l('added_successfully', _l('pjk3_group')) : '';
                echo json_encode([
                ...
                $message = '';
                if ($success == true) {
                    $message = _l('updated_successfully', _l('pjk3_group'));
                }
                echo json_encode([
                    'success' => $success,
                    'message' => $message,
    ...
    public function delete_group($id)
    {
        ...
        if (!$id) {
            redirect(admin_url('pjk3/groups'));
        }
        $response = $this->clients_model->delete_group($id);
        if ($response == true) {
            set_alert('success', _l('deleted', _l('pjk3_group')));
        } else {
            set_alert('warning', _l('problem_deleting', _l('pjk3_group_lowercase')));
        }
        redirect(admin_url('pjk3/groups'));
    }
    ...
    public function bulk_action()
    {
        ...
        if ($this->input->post('mass_delete')) {
            set_alert('success', _l('total_pjk3_deleted', $total_deleted));
        }
    }
    ...
    public function statement_pdf()
    {
        ...
        if (!has_permission('invoices', '', 'view') && !has_permission('payments', '', 'view')) {
            set_alert('danger', _l('access_denied'));
            redirect(admin_url('pjk3/client/' . $customer_id));
        }
    ...
    public function send_statement()
    {
        ...
        redirect(admin_url('pjk3/client/' . $customer_id . '?group=statement'));
    }
    ...
    public function statement()
    {
        ...
        $viewData['html'] = $this->load->view('admin/pjk3/groups/_statement', $data, true);

        echo json_encode($viewData);
    }


    public function delete_item($id,$client_id){
        if (!has_permission('items')) {
            set_alert('warning', _l('access_denied'));
            redirect(admin_url());
        }
        if (!$id) {
            redirect(admin_url('pjk3/client/'.$client_id));
        }
    ...
    function create_product_and_services($id = ''){
            ...
            if ($id == '') {
               ...
               if (!has_permission('items')) {
                ...
                if ($id) {
                    ...
                    //redirect(admin_url('pjk3/client/'.$userid));
                    echo json_encode([
                    'success' => true,
                    'message' =>   _l('added_successfully', _l('items')),
                ]);
                die;
                }
            } else {
                ...
                if ($success) {
                    ...
                    echo json_encode([
                    'success' => true,
                    'message' =>  _l('updated_successfully', _l('items')),
                ]);
                die;
                }
